...
public class TermuxPromptManager {
    ...
    public static boolean applyPromptStyle(Context context, TermuxActivity activity, PromptItem item) {
        ...
        try {
            // 2. 写入 ~/.termux/prompt.sh
            File homeDir = new File(context.getFilesDir(), "home");
            File termuxDir = new File(homeDir, ".termux");
            if (!termuxDir.exists()) termuxDir.mkdirs();

            File promptFile = new File(termuxDir, "prompt.sh");
            String content = "#!/bin/bash\n# Generated by Termux+ Style Manager\n" + item.ps1Script + "\nexport PS1\n";
            try (FileOutputStream fos = new FileOutputStream(promptFile)) {
                fos.write(content.getBytes(StandardCharsets.UTF_8));
            }

            // 3. 确保 ~/.bashrc 中加载 ~/.termux/prompt.sh
            File bashrcFile = new File(homeDir, ".bashrc");
            String bashrcSnippet = "\n[ -f ~/.termux/prompt.sh ] && . ~/.termux/prompt.sh\n";
            boolean needAppend = true;
            if (bashrcFile.exists()) {
                byte[] bytes = new byte[(int) bashrcFile.length()];
                try (java.io.FileInputStream fis = new java.io.FileInputStream(bashrcFile)) {
                    fis.read(bytes);
                }
                String existing = new String(bytes, StandardCharsets.UTF_8);
                if (existing.contains("prompt.sh")) {
                    needAppend = false;
                }
            }
            if (needAppend) {
                try (FileOutputStream fos = new FileOutputStream(bashrcFile, true)) {
                    fos.write(bashrcSnippet.getBytes(StandardCharsets.UTF_8));
                }
            }

            // 4. 真正毫秒级热加载：向活跃终端 Session 实时写入热生效指令
                ...
                if (termuxSessions != null) {
                    for (TermuxSession tSession : termuxSessions) {
                        ...
                        if (ts != null && ts.isRunning()) {
                            // 注入加载脚本，终端当前命令行立刻热刷新换装
                            ts.write(". ~/.termux/prompt.sh\n");
                        }
                    }
                }
            }
